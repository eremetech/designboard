...
export function registerPrompts(server: McpServer): void {
  ...
  server.prompt(
    'implement_component',
    'Generate a complete implementation prompt for a design pattern in a specific framework',
    {
  ...
  server.prompt(
    'review_against_pattern',
    'Review code against a canonical design pattern and flag deviations or improvements',
    {
  ...
  server.prompt(
    'suggest_design_system',
    'Given a UI description, suggest relevant design patterns from the catalog to build it',
    {
      description: z.string().describe(
        'Description of the UI you want to build (e.g. "e-commerce product page with hero, cards, and checkout")'
      )
    },
    async ({ description }) => {
      const keywords = description.toLowerCase().split(/\s+/).filter(w => w.length > 2);
      const accumulated = new Map<string, { slug: string; name: string; category: string; prompt: string; score: number; keywordHits: number }>();
