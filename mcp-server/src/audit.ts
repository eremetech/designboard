import { findPatternByName, suggestPatternNames, type Pattern, type Category } from './utils.js';
...
function extractCssText(code: string): string {
  const parts: string[] = [];

  // Inline styles: style="..." and style='...'
  for (const m of code.matchAll(/style\s*=\s*"([^"]*)"/gi)) parts.push(m[1]);
  for (const m of code.matchAll(/style\s*=\s*'([^']*)'/gi)) parts.push(m[1]);

  // <style> blocks
  for (const m of code.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)) parts.push(m[1]);
 ...
 */
function findGradient(css: string, type: string): string | null {
  ...
  for (let i = start + needle.length - 1; i < css.length; i++) {
    ...
    if (css[i] === ')') {
      depth--;
      if (depth === 0) return css.substring(start, i + 1);
    }
  }
  return null;
}
...
// ── Trait detectors ──────────────────────────────────────────────────
//
// Each detector looks for one visual technique in a CSS text blob.
// tier 1 = pattern-defining (backdrop-blur, gradients, shadows, filters)
// tier 2 = supporting (border-radius, transitions, overflow)

interface Detector {
  id: string;
  label: string;
  tier: 1 | 2;
  extract(css: string): string | null;
  compare(ref: string, found: string): 'pass' | 'partial';
  describeFix(ref: string): string;
}
...
const DETECTORS: Detector[] = [
  {
    ...
    describeFix: ref =>
      `Add \`backdrop-filter: ${ref}; -webkit-backdrop-filter: ${ref};\` to create the frosted glass effect`,
  },
  {
    ...
    describeFix: ref =>
      `Append \`${ref}\` to your backdrop-filter, e.g. \`backdrop-filter: blur(…) ${ref};\``,
  },
  {
    id: 'transparent-bg',
    ...
    extract: css => {
      for (const m of css.matchAll(/background(?:-color)?\s*:[^;]*rgba\s*\(([^)]+)\)/gi)) {
        ...
        if (alpha < 0.95) return `rgba(${parts.join(', ')})`;
      }
      return null;
    },
    compare: (ref, found) => {
  ...
  },
  {
    ...
    compare: () => 'pass',
    describeFix: () =>
      `Add \`overflow: hidden;\` to contain blur and child elements within the boundary`,
  },
];

// ── Scoring ──────────────────────────────────────────────────────────

const TIER_WEIGHT = { 1: 2, 2: 1 } as const;
const PARTIAL_FACTOR = 0.5;

function auditOnePattern(
  submittedCss: string,
  pattern: Pattern,
  category: Category,
): PatternAudit {
  ...
  const signature: { detector: Detector; refValue: string }[] = [];
  for (const d of DETECTORS) {
    const v = d.extract(refCss);
    if (v !== null) signature.push({ detector: d, refValue: v });
  }

  let totalWeight = 0;
  let earnedWeight = 0;
  const traits: TraitCheck[] = [];
  ...
  const verdict: PatternAudit['verdict'] =
...
export function auditImplementation(code: string, patternNames: string[]): AuditResult {
  const submittedCss = extractCssText(code);
  const audits: PatternAudit[] = [];
  const notFound: AuditResult['notFound'] = [];
