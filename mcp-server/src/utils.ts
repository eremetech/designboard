...
const data: PatternData = parsed.data;
...
export function findPatternBySlug(slug: string): { pattern: Pattern; category: Category } | undefined {
...
export function findPatternByName(name: string): { pattern: Pattern; category: Category } | undefined {
...
// ── BM25F inverted index (built once at load time) ───────────────────

type Field = 'name' | 'aliases' | 'category' | 'textual';
const FIELDS: Field[] = ['name', 'aliases', 'category', 'textual'];
const FIELD_BOOST: Record<Field, number> = { name: 3, aliases: 2, category: 1.5, textual: 1 };
const BM25_K1 = 1.2;
const BM25_B = 0.75;
...
// term → Map<docIndex, tf-per-field>
type FieldTFs = Record<Field, number>;
type PostingList = Map<number, FieldTFs>;

const docs: DocEntry[] = [];
const invertedIndex = new Map<string, PostingList>();
const avgFieldLen: Record<Field, number> = { name: 0, aliases: 0, category: 0, textual: 0 };
const docsByCategory = new Map<string, number[]>();

// Exact-match lookup tables
const exactName = new Map<string, number>();
const exactSlug = new Map<string, number>();

function addToIndex(docIdx: number, field: Field, tokens: string[]) {
...
function idf(docFreq: number): number {
  const N = docs.length;
 ...
 */
function bm25fTermScore(docIdx: number, tfs: FieldTFs, termIdf: number): number {
...
export function suggestCategorySlugs(input: string, limit = 3): string[] {
...
export function suggestPatternNames(input: string, limit = 3): { slug: string; name: string; category: string }[] {
  const lower = input.toLowerCase();
  const candidates: { slug: string; name: string; category: string; dist: number }[] = [];
  for (const category of data.categories) {
...
export function searchPatterns(
  ...
  for (const term of terms) {
    ...
    for (const [docIdx, tfs] of posting) {
      if (allowedDocs && !allowedDocs.has(docIdx)) continue;
