...
export function registerResources(server: McpServer): void {
  ...
  server.resource(
    'catalog',
    'designboard://catalog',
    { description: 'Full catalog of design pattern categories with names, descriptions, and pattern counts' },
    async () => ({
      contents: [{
  ...
  server.resource(
    'category',
    new ResourceTemplate('designboard://categories/{slug}', {
      list: async () => ({
    ...
    { description: 'All patterns in a design category, including names, aliases, and prompts' },
    async (_uri, { slug }) => {
      const category = findCategoryBySlug(slug as string);
      if (!category) {
        return {
          contents: [{
  ...
  server.resource(
    'pattern',
    new ResourceTemplate('designboard://patterns/{slug}', {
      list: undefined,
      complete: {
        slug: (value) => {
          ...
          for (const cat of getCategories()) {
            for (const p of cat.patterns) {
              if (p.slug.startsWith(value)) results.push(p.slug);
            }
          }
    ...
    { description: 'Single design pattern with full textual, HTML, and TSX prompts' },
    async (_uri, { slug }) => {
      const result = findPatternBySlug(slug as string);
      if (!result) {
        return {
          contents: [{
