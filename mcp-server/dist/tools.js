import { z } from 'zod';
import { getCategories, findCategoryBySlug, findPatternByName, searchPatterns, suggestCategorySlugs, suggestPatternNames, } from './utils.js';
import { createRequestLogger } from './logger.js';
import { auditImplementation } from './audit.js';
function patternNotFoundMessage(input) {
...
function categoryNotFoundMessage(input) {
...
export function registerTools(server) {
    // --- list_categories ---
    server.tool('list_categories', 'List all design pattern categories with descriptions and pattern counts', {}, async () => {
    ...
    // --- list_patterns ---
    server.tool('list_patterns', 'List all patterns in a category. Use list_categories first to get available category slugs.', {
    ...
    // --- search_patterns ---
    server.tool('search_patterns', 'Search design patterns by name, alias, or description. Returns matching patterns ranked by relevance.', {
    ...
    // --- get_pattern_prompt ---
    server.tool('get_pattern_prompt', 'Get the prompt for a specific design pattern in a chosen format (textual description, HTML code, or TSX/React code)', {
        pattern: z.string().describe('Pattern name or slug (e.g. "circle", "rounded-rectangle", "Card")'),
        format: z.enum(['textual', 'html', 'tsx']).describe('Prompt format: "textual" for natural-language description, "html" for HTML/CSS snippet, "tsx" for React component')
        ...
        if (!result) {
            ...
            return {
                content: [{ type: 'text', text: patternNotFoundMessage(pattern) }],
                isError: true
            };
        }
    ...
    // --- get_implementation ---
    server.tool('get_implementation', 'Get the implementation code for a design pattern along with its textual prompt as guidance. Useful for generating or scaffolding UI components.', {
    ...
    server.tool('audit_implementation', [
        ...
        patterns: z.array(z.string()).min(1).describe('Pattern names or slugs to check against (e.g. ["liquid-glass-card", "radial-gradient"])'),
    }, async ({ code, patterns }) => {
